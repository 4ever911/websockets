    ...
    * as an iterable of (name, value) pairs in the :attr:`raw_request_headers`
    ...
    def close(self, code=1000, reason=''):
        ...
        if self.state == OPEN:
            ...
            yield trollius.From(self.write_frame(OP_CLOSE, frame_data))

        # If the connection doesn't terminate within the timeout, break out of
        # the worker loop.
        try:
            yield trollius.From(trollius.wait_for(self.worker_task, self.timeout, loop=self.loop))
        except trollius.TimeoutError:
            self.worker_task.cancel()

        # The worker should terminate quickly once it has been cancelled.
        yield trollius.From(self.worker_task)
    ...
    def recv(self):
        ...
        try:
            done, pending = yield trollius.From(trollius.wait([next_message, self.worker_task],loop=self.loop, return_when=trollius.FIRST_COMPLETED))
    ...
    def send(self, data):
        ...
        """
        yield trollius.From(self.ensure_open())
        ...
        yield trollius.From(self.write_frame(opcode, data))
    ...
    def ping(self, data=None):
        ...
        yield trollius.From(self.write_frame(OP_PING, data))
        raise trollius.Return(self.pings[data])
    ...
    def pong(self, data=b''):
        ...
        yield trollius.From(self.write_frame(OP_PONG, data))
    ...
    def ensure_open(self):
        ...
        if self.state == CLOSING:
            yield trollius.From(trollius.wait_for(self.worker_task, 3 * self.timeout, loop=self.loop))
            raise ConnectionClosed(self.close_code, self.close_reason)
    ...
    def run(self):
        ...
        yield trollius.From(self.opening_handshake)
        while not self.closing_handshake.done():
            try:
                msg = yield trollius.From(self.read_message())
                if msg is None:
                    break
                yield trollius.From(self.messages.put(msg))
            except trollius.CancelledError:
                break
            except WebSocketProtocolError:
                yield trollius.From(self.fail_connection(1002))
            except trollius.IncompleteReadError:
                yield trollius.From(self.fail_connection(1006))
            except UnicodeDecodeError:
                yield trollius.From(self.fail_connection(1007))
            except PayloadTooBig:
                yield trollius.From(self.fail_connection(1009))
            except Exception:
                yield trollius.From(self.fail_connection(1011))
                raise
        yield trollius.From(self.close_connection())

    @trollius.coroutine
    def read_message(self):
        # Reassemble fragmented messages.
        frame = yield trollius.From(self.read_data_frame(max_size=self.max_size))
        if frame is None:
            return
        ...
        if frame.fin:
            raise trollius.Return(frame.data.decode('utf-8') if text else frame.data)
        ...
        while not frame.fin:
            frame = yield trollius.From(self.read_data_frame(max_size=max_size[0]))
            if frame is None:
        ...
        raise trollius.Return(('' if text else b'').join(chunks))
    ...
    def read_data_frame(self, max_size):
        ...
        # 6.2. Receiving Data
        while True:
            frame = yield trollius.From(self.read_frame(max_size))
            ...
            if frame.opcode == OP_CLOSE:
                ...
                if self.state == OPEN:
                    # 7.1.3. The WebSocket Closing Handshake is Started
                    yield trollius.From(self.write_frame(OP_CLOSE, frame.data))
                ...
                # Answer pings.
                yield trollius.From(self.pong(frame.data))
            ...
            # 5.6. Data Frames
                ...
                raise trollius.Return(frame)

    @trollius.coroutine
    def read_frame(self, max_size):
        is_masked = not self.is_client
        frame = yield trollius.From(read_frame(self.reader.readexactly, is_masked, max_size=max_size))
    ...
    def write_frame(self, opcode, data=b''):
        ...
        try:
            ...
            yield trollius.From(self.writer.drain())
    ...
    def close_connection(self, force=False):
        ...
        if self.is_client and not force:
            try:
                yield trollius.From(trollius.wait_for(self.connection_closed, self.timeout, loop=self.loop))
