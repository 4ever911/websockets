...
class WebSocketServerProtocol(WebSocketCommonProtocol):
    ...
    def handler(self):
        ...
        try:
            ...
            try:
                path = yield trollius.From(self.handshake(origins=self.origins, subprotocols=self.subprotocols,extra_headers=self.extra_headers))
            ...
            try:
                yield trollius.From(self.ws_handler(self, path))
            except Exception as exc:
                if self._is_server_shutting_down(exc):
                    yield trollius.From(self.fail_connection(1001))
                else:
            ...
            try:
                yield trollius.From(self.close())
            except Exception as exc:
                if self._is_server_shutting_down(exc):
                    pass
                else:
    ...
    def handshake(self, origins=None, subprotocols=None, extra_headers=None):
        ...
        try:
            path, headers = yield trollius.From(read_request(self.reader))
...
class WebSocketServer(trollius.AbstractServer):
    ...
    def wait_closed(self):
        ...
        if self.websockets:
            ...
            yield trollius.From(trollius.wait([websocket.handler_task for websocket in self.websockets] +[websocket.worker_task for websocket in self.websockets],loop=self.loop))
        yield trollius.From(self.server.wait_closed())
...
def serve(ws_handler, host=None, port=None, tmp=None,
    ...
    * a :meth:`~trollius.Server.wait_closed` coroutine that waits until closing
    ...
    server = yield trollius.From(loop.create_server(factory, host, port, **kwds))

    ws_server.wrap(server)

    raise trollius.Return(ws_server)
