...
class WebSocketClientProtocol(WebSocketCommonProtocol):
    ...
    def handshake(self, wsuri,
        ...
        try:
            status_code, headers = yield trollius.From(read_response(self.reader))
...
def connect(uri,
    ...
    * ``subprotocols`` is a list of supported subprotocols in order of
    ...
    transport, protocol = yield trollius.From(loop.create_connection(factory, wsuri.host, wsuri.port, **kwds))

    try:
        yield trollius.From(protocol.handshake( wsuri,
                                                origin=origin,
                                                subprotocols=subprotocols,
                                                extra_headers=extra_headers))
    except Exception:
        yield trollius.From(protocol.close_connection(force=True))
        raise
