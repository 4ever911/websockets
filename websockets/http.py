...
def read_request(stream):
    ...
    """
    request_line, headers = yield trollius.From(read_message(stream))
    method, path, version = request_line[:-2].decode().split(None, 2)
    if method != 'GET':
        raise ValueError("Unsupported method")
    if version != 'HTTP/1.1':
        raise ValueError("Unsupported HTTP version")
    raise trollius.Return(path, headers)
...
def read_response(stream):
    ...
    """
    status_line, headers = yield trollius.From(read_message(stream))
    version, status, reason = status_line[:-2].decode().split(None, 2)
    if version != 'HTTP/1.1':
        raise ValueError("Unsupported HTTP version")
    raise trollius.Return(int(status), headers)
...
def read_message(stream):
    ...
    """
    start_line = yield trollius.From(read_line(stream))
    header_lines = io.BytesIO()
    for num in range(MAX_HEADERS):
        header_line = yield trollius.From(read_line(stream))
...
def read_line(stream):
    ...
    """
    line = yield trollius.From(stream.readline())
    if len(line) > MAX_LINE:
        raise ValueError("Line too long")
    if not line.endswith(b'\r\n'):
        raise ValueError("Line without CRLF")
    raise trollius.Return(line)
